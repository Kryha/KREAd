...
const executeProposal = async (powers) => {
  ...
  const {
    consume: { agoricNamesAdmin, board, chainStorage, zoe, chainTimerService },
  ...
  try {
    const [
      [itemBrand, itemIssuer],
      [characterIssuer, characterBrand],
      [tokenIssuer, tokenBrand],
    ] = await Promise.all([
      ...
      reserveThenGetNamePaths(agoricNamesAdmin, [
        ['brand', 'kreadCharacter'],
        ['issuer', 'kreadCharacter'],
      ]),
      reserveThenGetNamePaths(agoricNamesAdmin, [
        ['brand', 'kreadPaymentFT'],
        ['issuer', 'kreadPaymentFT'],
      ]),
    ]);
    console.log(itemBrand, itemIssuer);
  } catch (e) {
    console.log(e);
  }

  const nameHub = E(agoricNamesAdmin).readonly();
  console.log(E(nameHub).lookup('kreadItem'));
  ...
  const instanceKey = `kread`;
  const instanceAdmin = E(agoricNamesAdmin).lookupAdmin('instance');

  await E(instanceAdmin).update(instanceKey, kreadKit);
...
//# sourceURL=/Users/carlostrigoseguin/Documents/KRYHA/agoric/REPO/agoric/contract/src/proposal/deploy-kread.js
