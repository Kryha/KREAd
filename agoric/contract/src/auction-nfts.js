...
const start = (zcf) => {
  const {
    ...
    timeAuthority,
    winnerPriceOption,
    bidDuration = 1n, // 5 mins in chain timer 1s resolution
    auctionInstallation,
  } = zcf.getTerms();
  assertIssuerKeywords(zcf, harden(['CB', 'Money']));

  const moneyBrand = brands.Money;
  const characterBrand = brands.CB;

  assertNatAssetKind(zcf, moneyBrand);

  /** @type Record<string, AuctionSession> */
  const sellerSessions = {};
  let availableItems = AmountMath.make(characterBrand, harden([]));
  ...
  const sell = (seat) => {
    ...
    const addedAmount = sellerSeat.getAmountAllocated('CB', characterBrand);
  ...
  const startAuctioningItem = async (itemKey) => {
    const itemAmount = AmountMath.make(characterBrand, harden([itemKey]));
    const availableAmount = sellerSeat.getAmountAllocated('CB', characterBrand);
  ...
  const getSessionDetailsForKey = async (item) => {
    ...
    if (!session) {
      ...
      const itemAmount = AmountMath.make(characterBrand, harden([item]));
