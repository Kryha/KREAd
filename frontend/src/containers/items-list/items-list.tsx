...
export const ItemsList: FC<Props> = ({ onItemClick }) => {
  ...
  return (
    <SortableListWrap>
      ...
      <HorizontalDivider />
      <ListContainer>
        <MenuItem data={{ ...items[0], image: items[0].icon }}  key={items[0].id} onClick={() => onItemClick(items[0].id)} isInitial={intitial} removeInitial={removeInitial} />
        {items.slice(1).map((item) => (
          <MenuItem data={{ ...item, image: item.icon }}  key={item.id} onClick={() => onItemClick(item.id)} removeInitial={removeInitial} />
        ))}
      </ListContainer>
    </SortableListWrap>
  );
